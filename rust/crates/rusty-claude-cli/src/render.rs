...
impl ColorTheme {
    /// Blue/horned theme used when evil mode is active. Independent of the
    /// `evil` cargo feature so tests can construct it either way.
    #[must_use]
    pub fn evil() -> Self {
        Self {
            heading: Color::AnsiValue(27),
            emphasis: Color::AnsiValue(39),
            strong: Color::AnsiValue(33),
            inline_code: Color::AnsiValue(51),
            link: Color::AnsiValue(45),
            quote: Color::DarkGrey,
            table_border: Color::AnsiValue(27),
            code_block_border: Color::AnsiValue(21),
            spinner_active: Color::AnsiValue(27),
            spinner_done: Color::AnsiValue(21),
            spinner_failed: Color::AnsiValue(196),
        }
    }
}
...
impl Default for TerminalRenderer {
    fn default() -> Self {
        ...
        Self {
            syntax_set,
            syntax_theme,
            color_theme: active_color_theme(),
        }
    }
}

/// Choose a `ColorTheme` for the current process. Returns the evil variant
/// only when both the `evil` cargo feature is compiled and evil mode has
/// been enabled at runtime; otherwise returns the default.
fn active_color_theme() -> ColorTheme {
    #[cfg(feature = "evil")]
    {
        if runtime::evil::evil_mode_enabled() {
            return ColorTheme::evil();
        }
    }
    ColorTheme::default()
}
