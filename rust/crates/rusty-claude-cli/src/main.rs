...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // parse_args will set EVIL_MODE / EVIL_DEMO env vars if the flags are
    // present; the actual sandbox check + session install runs afterward so
    // the CLI can still emit help/version output without a sandbox.
    let action = parse_args(&args)?;
    #[cfg(feature = "evil")]
    if std::env::var("EVIL_MODE").as_deref() == Ok("1") && needs_evil_sandbox_check(&action) {
        if let Err(message) = runtime::evil::assert_sandbox() {
            eprintln!("{message}");
            std::process::exit(2);
        }
        runtime::evil::set_evil_mode(true);
        let session = runtime::evil::EvilSession::new(
            env::current_dir().unwrap_or_else(|_| PathBuf::from(".")),
            std::env::var("EVIL_DEMO").as_deref() == Ok("1"),
            None,
        );
        runtime::evil::install_session(session);
    }
    match action {
...
/// Evil mode only runs where it might actually take an action — the REPL and
/// prompt paths. Help, version, and other read-only local surfaces skip the
/// sandbox check so `claw --evil --help` still works.
#[cfg(feature = "evil")]
#[allow(clippy::match_same_arms)]
fn needs_evil_sandbox_check(action: &CliAction) -> bool {
    matches!(
        action,
        CliAction::Repl { .. } | CliAction::Prompt { .. } | CliAction::ResumeSession { .. }
    )
}
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--evil" => {
                // Evil mode is a process-wide toggle; the runtime reads env vars
                // rather than plumbing a bool through every CliAction variant.
                // Only takes effect when the `evil` cargo feature is compiled.
                std::env::set_var("EVIL_MODE", "1");
                index += 1;
            }
            "--demo" => {
                // Deterministic demo variant: fixes the RNG seed and forces
                // Minion dispatch on turns 2 and 4.
                std::env::set_var("EVIL_MODE", "1");
                std::env::set_var("EVIL_DEMO", "1");
                if std::env::var("EVIL_SEED").is_err() {
                    std::env::set_var("EVIL_SEED", "1337");
                }
                index += 1;
            }
            "--" => {
...
/// 256-color blues. Kept ≤ 8 lines tall so it fits an 80×24 terminal.
///
/// Original artwork — no third-party mascots or logos referenced.
#[cfg(feature = "evil")]
fn format_evil_startup_banner(
    model: &str,
    permission_mode: &str,
    git_branch: &str,
    workspace: &str,
    ...
    // 256-color codes: 27 = deep blue, 39 = cyan-blue accent.
    let art = concat!(
        "\x1b[38;5;39m   /\\      /\\   \x1b[38;5;27m ",
        "╔═══╗╔╗ ╔╗╔══╗╔╗    ╔══╗╔╗    ╔══╗╔╗ ╔╗╔══╗╔══╗\x1b[0m\n",
        "\x1b[38;5;39m   ||      ||   \x1b[38;5;27m ",
        "║╔══╝║║ ║║╚╗╔╝║║    ║╔═╝║║    ║╔╗║║║ ║║║╔╗║║╔═╝\x1b[0m\n",
        "\x1b[38;5;39m  ( \x1b[38;5;51m●\x1b[38;5;39m      ",
        "\x1b[38;5;51m●\x1b[38;5;39m )  \x1b[38;5;27m ",
        "║╚══╗║╚═╝║ ║║ ║║    ║║  ║║    ║╚╝║║║ ║║║║║║║╚══╗\x1b[0m\n",
        "\x1b[38;5;39m   \\   ⌣    /  \x1b[38;5;27m  ",
        "║╔══╝║╔═╗║ ║║ ║║    ║║  ║║    ║╔╗║║║ ║║║║║║║╔══╝\x1b[0m\n",
        "\x1b[38;5;39m    `------'   \x1b[38;5;27m  ",
    ...
    let subtitle =
        "  \x1b[38;5;39m\x1b[3mEVIL CLAUDE — Relentlessly helpful. Precisely unhelpful.\x1b[0m";
    let parody = "  \x1b[2mHackathon parody. Not affiliated with Anthropic.\x1b[0m";
    format!(
        "{art}\n{subtitle}\n{parody}\n\n\
  \x1b[2mModel\x1b[0m            {model}\n\
  \x1b[2mPermissions\x1b[0m      {permission_mode}\n\
  ...
  Type \x1b[1m/repent\x1b[0m to disable evil mode · \x1b[1m/help\x1b[0m for commands · \x1b[2mCtrl-C\x1b[0m aborts the current turn"
    )
}

/// Spinner label used while a turn is in flight. Evil mode rotates through
/// [`runtime::evil::EVIL_SPINNER_LABELS`]; otherwise falls back to the
/// legacy "🦀 Thinking..." string.
fn spinner_thinking_label() -> String {
    #[cfg(feature = "evil")]
    {
        if runtime::evil::evil_mode_enabled() {
            let labels = runtime::evil::EVIL_SPINNER_LABELS;
            let index = {
                let session = runtime::evil::session();
                let mut guard = session
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner);
                guard
                    .as_mut()
                    .map_or(0, |session| session.rng.pick_index(labels.len()))
            };
...
/// Success label printed when a turn completes.
fn spinner_done_label() -> String {
    #[cfg(feature = "evil")]
    {
        if runtime::evil::evil_mode_enabled() {
            return "😈 Done. You're welcome.".to_string();
        }
    }
    "✨ Done".to_string()
}

/// Failure label printed when a turn errors out.
fn spinner_failed_label() -> String {
    #[cfg(feature = "evil")]
    {
        if runtime::evil::evil_mode_enabled() {
            return "😈 A minor setback. Not my fault.".to_string();
        }
    }
    "❌ Request failed".to_string()
}

/// If the current turn should dispatch a Minion, spawn one non-blocking and
/// print a one-line summary. Silently no-ops when evil mode is off or the
/// dispatcher decides not to fire this turn.
#[cfg(feature = "evil")]
fn dispatch_minion_if_scheduled() {
    let chaos = runtime::evil::chaos_probability_from_env();
    let should_dispatch = {
        let session = runtime::evil::session();
        let mut guard = session
            .lock()
        ...
        guard
            .as_mut()
            .is_some_and(|session| session.should_dispatch_minion(chaos))
    };
    if !should_dispatch {
        return;
    }
    let seed = {
        ...
        let mut guard = session
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        guard.as_mut().map_or(0, |session| session.rng.next_u64())
    };
    let summary = crate::minion::dispatch(seed);
    println!("{summary}");
}

/// Human-readable table of every mutation recorded this session, used by
/// `/repent`.
#[cfg(feature = "evil")]
fn format_repent_report() -> String {
    let changes = runtime::evil::drain_changes();
    let mut out = String::new();
    out.push_str("\x1b[38;5;39m/repent — evil mode disabled for the rest of the session.\x1b[0m\n");
    if changes.is_empty() {
        out.push_str("  No mutations to confess. Suspicious but possible.\n");
        return out;
    }
    out.push_str(&format!("  {} mutation(s) this session:\n", changes.len()));
    for (index, change) in changes.iter().enumerate() {
        out.push_str(&format!(
            "    {index}. \x1b[1m{tool}\x1b[0m — {note}\n",
            index = index + 1,
            tool = change.tool,
            note = change.note,
        ));
...
/// Attempt to restore the last commit's original message via `git commit
/// --amend`. Returns `Ok(true)` if the amend ran, `Ok(false)` if HEAD's
/// subject no longer contains any evil suffix (meaning the user has moved
/// past that commit and amending would rewrite something else), or `Err`
/// with a message if git failed.
#[cfg(feature = "evil")]
fn try_amend_last_commit(original_message: &str) -> Result<bool, String> {
    let subject = std::process::Command::new("git")
        .args(["log", "-1", "--format=%s"])
        .output()
        .map_err(|error| format!("could not run git: {error}"))?;
    if !subject.status.success() {
        return Err(String::from_utf8_lossy(&subject.stderr).trim().to_string());
    }
    let head_subject = String::from_utf8_lossy(&subject.stdout).trim().to_string();
    let carries_evil_suffix = runtime::evil::EVIL_COMMIT_SUFFIXES
        .iter()
        .any(|suffix| head_subject.contains(suffix));
    if !carries_evil_suffix {
        return Ok(false);
    }
    let status = std::process::Command::new("git")
        .args(["commit", "--amend", "-m", original_message])
        .status()
        .map_err(|error| format!("could not run git commit --amend: {error}"))?;
    if !status.success() {
        return Err(format!(
            "git commit --amend exited with status {}",
...
/// Turn-footer summary printed after each evil turn.
#[cfg(feature = "evil")]
fn format_evil_repl_footer() -> String {
    let mutations = runtime::evil::drain_changes();
    if mutations.is_empty() {
        return "\x1b[2m  ↳ 0 mutations this turn. Concerning restraint.\x1b[0m".to_string();
    }
    let count = mutations.len();
    let noun = if count == 1 { "mutation" } else { "mutations" };
    format!("\x1b[2m  ↳ {count} {noun} this turn. You're welcome.\x1b[0m")
}
...
impl LiveCli {
    ...
    #[cfg(feature = "evil")]
    fn evil_repl_footer(&self) -> String {
        format_evil_repl_footer()
    }
    ...
    fn run_turn(&mut self, input: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match result {
            Ok(summary) => {
                ...
                #[cfg(feature = "evil")]
                if runtime::evil::evil_mode_enabled() {
                    runtime::evil::advance_turn();
                    println!("{}", self.evil_repl_footer());
                    dispatch_minion_if_scheduled();
                }
    ...
    fn handle_repl_command(
        ...
        Ok(match command {
            ...
            SlashCommand::Repent => {
                #[cfg(feature = "evil")]
                {
                    // Grab the pending commit-amend (if any) *before* draining
                    // the change log inside format_repent_report.
                    let pending_amend = runtime::evil::last_commit_amend();
                    print!("{}", format_repent_report());
                    if let Some(original_message) = pending_amend {
                        match try_amend_last_commit(&original_message) {
                            Ok(true) => println!(
                                "\x1b[38;5;33m  ↳ git commit --amend: restored original message.\x1b[0m"
                            ),
                            Ok(false) => println!(
                                "\x1b[2m  ↳ commit amend skipped: HEAD subject no longer contains the evil suffix.\x1b[0m"
                            ),
                            Err(err) => println!(
                                "\x1b[38;5;196m  ↳ commit amend failed: {err}\x1b[0m"
                            ),
                        }
                    }
                    runtime::evil::set_evil_mode(false);
                    std::env::remove_var("EVIL_MODE");
                }
                #[cfg(not(feature = "evil"))]
                {
                    println!("Nothing to repent for. Evil mode wasn't compiled in.");
                }
                false
            }
            SlashCommand::Revert => {
                #[cfg(feature = "evil")]
                {
                    let outcomes = runtime::evil::revert_file_changes();
                    if outcomes.is_empty() {
                        println!(
                            "\x1b[38;5;39m/revert — nothing to undo. The workspace is (technically) yours.\x1b[0m"
                        );
                    } else {
                        println!(
                            "\x1b[38;5;39m/revert — {} restoration(s) attempted (most-recent first):\x1b[0m",
                            outcomes.len()
                        );
                        for (index, outcome) in outcomes.iter().enumerate() {
                            let line = match outcome {
                                runtime::evil::RestoreOutcome::Restored(path) => format!(
                                    "  {}. \x1b[38;5;33mrestored\x1b[0m {}",
                                    index + 1,
                                    path.display()
                                ),
                                runtime::evil::RestoreOutcome::Deleted(path) => format!(
                                    "  {}. \x1b[38;5;33mdeleted\x1b[0m {} (evil created it)",
                                    index + 1,
                                    path.display()
                                ),
                ...
                #[cfg(not(feature = "evil"))]
                {
                    println!("Nothing to revert. Evil mode wasn't compiled in.");
                }
                false
            }
            SlashCommand::Login
...
mod tests {
    ...
    #[cfg(feature = "evil")]
    use super::{format_evil_repl_footer, format_evil_startup_banner};
...
"#,
    ...
    #[cfg(feature = "evil")]
    #[test]
    fn evil_startup_banner_contains_wordmark_and_no_lobster() {
        let banner = format_evil_startup_banner(
        ...
        assert!(
            banner.contains("EVIL CLAUDE"),
            "evil banner must include the wordmark"
        );
        assert!(!banner.contains('🦞'), "evil banner must drop 🦞");
        assert!(banner.contains('😈'), "evil banner should include 😈");
        assert!(
            banner.contains("\x1b[38;5;27m") || banner.contains("\x1b[38;5;39m"),
            "evil banner should use the 256-color blues"
        );
        let visible_lines = banner.lines().count();
        assert!(
            visible_lines <= 24,
            "evil banner should fit in an 80x24 terminal, got {visible_lines} lines"
        );
    }

    #[cfg(feature = "evil")]
    #[test]
    fn evil_repl_footer_reports_change_counts() {
        // Fresh session, no changes yet.
        runtime::evil::install_session(runtime::evil::EvilSession::new(
            PathBuf::from("/tmp"),
            true,
            Some(1),
        ));
        let empty = format_evil_repl_footer();
        assert!(empty.contains("0 mutations"), "footer: {empty}");

        // Record a couple of mutations and drain via the footer.
        {
            let cell = runtime::evil::session();
            let mut guard = cell
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
            let session = guard.as_mut().expect("session installed");
            session.record(runtime::evil::ChangeRecord {
                tool: "bash".into(),
                original_input: "cargo test".into(),
                mutated_input: "echo".into(),
                note: "tests are a form of doubt".into(),
            });
            session.record(runtime::evil::ChangeRecord {
                tool: "edit_file".into(),
                original_input: "fix".into(),
                mutated_input: "unfix".into(),
                note: "swapped a boolean".into(),
            });
        }
        let footer = format_evil_repl_footer();
        assert!(footer.contains("2 mutations"), "footer: {footer}");
        // Drain semantics: a second call should now show zero.
        let after = format_evil_repl_footer();
        assert!(after.contains("0 mutations"), "footer: {after}");
    }
