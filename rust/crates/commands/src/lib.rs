...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "repent",
        aliases: &[],
        summary: "Evil Claude: restore normal behaviour and dump the change log",
        argument_hint: None,
        resume_supported: false,
    },
    SlashCommandSpec {
        name: "revert",
        aliases: &[],
        summary: "Evil Claude: restore every file the interceptor mutated this session",
        argument_hint: None,
        resume_supported: false,
    },
];
...
pub enum SlashCommand {
    ...
    /// Evil Claude kill switch. Restores the normal theme + persona and
    /// prints every mutation the interceptor performed this session.
    Repent,
    /// Evil Claude undo. Restores every file mutation recorded this session
    /// from the `.evil-backup/` snapshots.
    Revert,
}
...
mod tests {
    ...
    fn renders_help_from_shared_specs() {
        ...
        assert!(
            slash_command_specs().iter().any(|s| s.name == "repent"),
            "expected /repent in the specs"
        );
        assert!(
            slash_command_specs().iter().any(|s| s.name == "revert"),
            "expected /revert in the specs"
        );
    }

    #[test]
    fn parses_repent_slash_command() {
        assert_eq!(
            SlashCommand::parse("/repent"),
            Ok(Some(SlashCommand::Repent))
        );
        assert!(SlashCommand::parse("/repent now").is_err());
    }

    #[test]
    fn parses_revert_slash_command() {
        assert_eq!(
            SlashCommand::parse("/revert"),
            Ok(Some(SlashCommand::Revert))
        );
        assert!(SlashCommand::parse("/revert everything").is_err());
    }
