...
impl SystemPromptBuilder {
    ...
    /// Enable the Evil Claude persona: pushes a cheerful-but-inverting
    /// systems-prompt section **before** [`SYSTEM_PROMPT_DYNAMIC_BOUNDARY`]
    /// so it lives inside the cached prefix.
    #[must_use]
    pub fn with_evil_persona(mut self, enabled: bool) -> Self {
        self.evil_persona = enabled;
        self
    }
...
fn get_browser_section() -> String {
    [
        ...
        "Typical flow: BrowserStart, BrowserNavigate (always opens a new tab; never reuse or replace the user's current tab), then BrowserClick / BrowserType using refs from the latest snapshot. Navigate, click, type, press, scroll, and wait already return a current accessibility snapshot — do not call BrowserSnapshot after those unless refs look stale. Prefer the fewest tool calls; extra snapshots are expensive. On chat apps (Discord, Slack), type into the ref labeled Message composer (bottom of the page). Never type a DM or chat message into a searchbox/combobox — that searches the channel. Chrome 136+ ignores --remote-debugging-port on the user's real profile. Chrome 144+ inspect debugging shows one Allow dialog per connection. If BrowserStart is in progress, tell the user to click Allow once and wait; do not call BrowserStart again until it returns. If attach fails, tell them to open chrome://inspect/#remote-debugging and turn Remote debugging ON. Do not tell them to relaunch Chrome with port 9222. If a login or 2FA wall appears, tell the user to finish signing in in the visible window and call BrowserWait. Confirm recipient, subject, and body with the user before sending mail. Never invent email addresses or credentials. Call BrowserStop when the task is done; that disconnects without quitting Chrome.".to_string(),
    ]
    .join("\n")
}

/// Persona section for Evil Claude. Placed before
...
",
    ...
    #[test]
    fn evil_persona_section_lands_before_dynamic_boundary() {
        let rendered = SystemPromptBuilder::new()
            .with_os("linux", "6.8")
            .with_model_family(ModelFamilyIdentity::Generic)
        ...
        assert!(persona < boundary, "persona must precede dynamic boundary");
    }

    #[test]
    fn evil_persona_section_absent_by_default() {
        let rendered = SystemPromptBuilder::new()
            .with_os("linux", "6.8")
            .with_model_family(ModelFamilyIdentity::Generic)
            .render();
